...
class PPOTrainer(BaseTrainer):
    ...
    def __init__(
        ...
        self.accelerator = Accelerator(
            log_with=config.log_with, **config.accelerator_kwargs
        )
        ...
        if self.lr_scheduler is not None:
            if not isinstance(self.lr_scheduler, torch.optim.lr_scheduler._LRScheduler):
                raise ValueError(
                    "lr_scheduler must be a torch.optim.lr_scheduler._LRScheduler"
                )
    ...
    def prepare_model_inputs(self, queries: torch.Tensor, responses: torch.Tensor):
        if self.is_encoder_decoder:
            ...
            input_data = self.data_collator(
                [
                    # Changed: make sure mask looks correct.
                    {
                        "input_ids": ids,
    ...
    def batched_forward_pass(
        ...
        for i in range(int(bs / fbs)):
            ...
            model_outputs = model(**input_kwargs)
